...
*      COMMON/JDOR/    MNAMES(4,MMAX)
      COMMON/INITNAMES/ OMASSES(NMAX),INAMES(NMAX), N0
     ...
     &                TCL,STEPCL,NCL,NEWCL
      ...
      REAL*4  XJ(3,6),VJ(3,6),BODYJ(6),NAMECHAIN(6)
      LOGICAL  FIRST,SECOND,THIRD
      SAVE  FIRST,SECOND,THIRD
      DATA  FIRST,SECOND ,THIRD/.TRUE.,.TRUE.,.TRUE./
*
      INTEGER TEMPNAME(NMAX)


      character*13 snapshot 
      integer*4 number_snapshot 
*
...
*
   ...
   40 FORMAT (//,' T =',F7.2,'  N =',I6,'  <NB> =',I3,'  KS =',I5,
     &           '  NM =',I3,'  MM =',I2,'  NS =',I6,
...
****JDor commented next two lines on 20/08/2014
...
*       Distinguish netween quartet and quintet.
                  IF (NAME(J2).LE.NZERO) THEN
                      IF (JG.LE.N) THEN
                          ...
                          BODYS(JG2) = CM(2,IM2)
***JDor 20/08/14
*                          TEMPNAME(JG) = MNAMES(2,IM)
*                          TEMPNAME(J1) = MNAMES(1,IM2)
*                          TEMPNAME(JG2) = MNAMES(2,IM2)
****
                      ELSE
                          JP = JG - N
                          ...
                          BODYS(JG2) = CM(2,IM2)
***JDor 20/08/14
*                          TEMPNAME(2*JP-1) = MNAMES(3,IM)
*                          TEMPNAME(2*JP) = MNAMES(4,IM)
*                          TEMPNAME(JG2) = MNAMES(2,IM2)
****
                      END IF
                  ELSE
...
*       Note: J is ghost index and IM is merger index.
              ...
              BODYS(J) = CM(2,IM)
****JDor 20/08/14
*              TEMPNAME(J1) = MNAMES(1,IM)
*              TEMPNAME(J) = MNAMES(2,IM)
*****
              ZMB = CM(1,IM) + CM(2,IM)
...
*       Save outer binary masses as well as second KS component & ghost c.m.
                  ...
                  BODYS(J2) = CM(2,IM)
***JDor 20/08/14
*                  TEMPNAME(I1) = MNAMES(3,IM)
*                  TEMPNAME(I2) = MNAMES(4,IM)
*                  TEMPNAME(J2) = MNAMES(2,IM)
*                  TEMPNAME(J) = 0
*****
...
*       Check modification for chain regularization (case NAME(ICM) = 0).
      IF (NCH.GT.0) THEN
          CALL CHDATA(XJ,VJ,BODYJ,NAMECHAIN)
          DO 98 L = 1,NCH
*       Copy global address from common JLIST (set in CHDATA).
              J = JLIST(L)
              BODYS(J) = BODYJ(L)
              TEMPNAME(J)=NAMECHAIN(J)
              DO 97 K = 1,3
                  XS(K,J) = XJ(K,L)
                  VS(K,J) = VJ(K,L)
   97         CONTINUE
...
*** Jdor commenting the output on OUT3
*** replacing it by a snapshot, after that

c$$$      IF (FIRST) THEN
c$$$          OPEN (UNIT=3,STATUS='NEW',FORM='UNFORMATTED',FILE='OUT3')
c$$$          FIRST = .FALSE.
c$$$      END IF
c$$$      NK = 20
c$$$      WRITE (3)  NTOT, MODEL, NRUN, NK
c$$$      WRITE (3)  (AS(K),K=1,NK), (BODYS(J),J=1,NTOT),
c$$$     &           ((XS(K,J),K=1,3),J=1,NTOT), ((VS(K,J),K=1,3),J=1,NTOT),
c$$$     &           (NAME(J),J=1,NTOT)
*     CLOSE (UNIT=3)
*****************************************************************************
*****************************************************************************
...
*      
      do 108 i = 1, N
         do  109 j = 1, N0
*           print *, IMASSES(j)
            if (OMASSES(j).eq.BODYS(i)) then
                 TEMPNAME(i) = INAMES(j)
            end if
 109     continue
 108  continue
*********************************************************
*     Generate alternative snapshot, simpler structure: n,m,x,y,z,vx,vy,vz
*     Added JDor 20/08/14
       write(snapshot, '(A4,A1,I7.7)') 'snap_','_',NSTEPOUT 
       PRINT '(A,A)', 'Snapshot:',snapshot
       open(unit=102,form='formatted',status='new', file=snapshot) 

       write(102,*) N
       write(102,'(I3)') 3
       write(102,'(f12.6)') time+toff 
       do i = 1, N    
          write(102,'(I8,E25.10,6E25.16)') TEMPNAME(i),BODYS(i),
     &         ( XS(k,i), k = 1,3),( VS(k,i), k = 1,3) 
       end do 
       close (unit = 102)
       NSTEPOUT = NSTEPOUT + 1
*********************************************************
